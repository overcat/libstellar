...
#include "../../src/fee_bump_transaction_envelope.h"
#include "../../src/memo.h"
...
void test_fee_bump_transaction_envelope_hash(void **state) {
  ...
  char expect_hash[] = {0x43, 0xa1, 0xb9, 0x4b, 0xab, 0x4f, 0x6,  0x9b,
                        0x85, 0xe9, 0x86, 0xfe, 0x4b, 0x65, 0xd6, 0x7d,
                        0xa4, 0x45, 0x9a, 0xd4, 0x5,  0x31, 0x8c, 0x31,
                        0xc0, 0xae, 0x69, 0x70, 0x1,  0x7,  0x56, 0x1e};
  char hash[32];
  assert_true(
      fee_bump_transaction_envelope_hash(&feeBumpTransactionEnvelope, hash));
  assert_memory_equal(expect_hash, hash, 32);
}

void test_fee_bump_transaction_envelope_sign_with_one_signer(void **state) {
  char *account = "GB777MJSHL7QOVZ3S52M2MROORDKLSJQL6MUSBV57R2NW7LODWORELJO";
  struct MuxedAccount muxed_account;
  assert_true(muxed_account_from_address(account, &muxed_account));
  struct Memo memo;
  ...
  char *seed3 = "SB4OH6ZGSW2QICVMEUO6FRCEDMN5OLMTZPI3YCLVRGK7BFWD3QBUF7WV";
  struct Keypair signer3;
  assert_true(keypair_from_secret(&signer3, seed3));
  assert_true(fee_bump_transaction_envelope_sign(&signer3,
                                                 &feeBumpTransactionEnvelope));
  assert_int_equal(feeBumpTransactionEnvelope.signatures_len, 1);
  struct DecoratedSignature expectDecoratedSignature3 = {
      .signatureHint = {0x2f, 0xd5, 0x85, 0xaa},
      .signature = {0x13, 0x17, 0xfe, 0x6a, 0xd8, 0x41, 0x6f, 0xe4, 0x1c, 0x19,
                    0xa0, 0xca, 0x6d, 0xdd, 0xd1, 0x8a, 0xf7, 0xe7, 0x84, 0x79,
                    0x15, 0x69, 0xd7, 0xa9, 0x8f, 0xfb, 0xdd, 0x95, 0x80, 0x6f,
                    0x68, 0xed, 0xd4, 0x53, 0xec, 0x7b, 0xf,  0xdc, 0xa9, 0x88,
                    0xea, 0xa7, 0xf5, 0xc3, 0xd,  0x60, 0xaa, 0x2e, 0x95, 0x53,
                    0xd3, 0x86, 0x64, 0x64, 0x7e, 0xb1, 0x3b, 0x61, 0x39, 0xd,
                    0x31, 0x0,  0xe6, 0x2}};

  assert_memory_equal(&expectDecoratedSignature3,
                      feeBumpTransactionEnvelope.signatures,
                      sizeof(struct DecoratedSignature));
}

void test_fee_bump_transaction_envelope_sign_with_two_signers(void **state) {
  char *account = "GB777MJSHL7QOVZ3S52M2MROORDKLSJQL6MUSBV57R2NW7LODWORELJO";
  struct MuxedAccount muxed_account;
  assert_true(muxed_account_from_address(account, &muxed_account));
  struct Memo memo;
  ...
  char *seed4 = "SBNVCVWT55XZIZFERJIIXH5Q6G4F7SCK26BRVCTRYCJ7RXGPAL3Y7OYG";
  struct Keypair signer4;
  assert_true(keypair_from_secret(&signer4, seed4));
  assert_true(fee_bump_transaction_envelope_sign(&signer4,
                                                 &feeBumpTransactionEnvelope));
  assert_int_equal(feeBumpTransactionEnvelope.signatures_len, 2);
  struct DecoratedSignature expectDecoratedSignature4 = {
      .signatureHint = {0x9e, 0xf1, 0x96, 0xaa},
      .signature = {0x1e, 0xc3, 0x50, 0x51, 0x32, 0x28, 0x25, 0x8b, 0xa5, 0x2a,
                    0x11, 0x8d, 0x14, 0x86, 0x6d, 0x6d, 0xc2, 0xb8, 0x63, 0x92,
                    0xd2, 0x39, 0xa8, 0x52, 0x93, 0x68, 0x2a, 0xf7, 0x67, 0x90,
                    0x2c, 0x92, 0x52, 0x3e, 0x79, 0xf6, 0x89, 0xb7, 0x78, 0x24,
                    0x47, 0x33, 0xc9, 0x8d, 0x33, 0x15, 0xc4, 0x8c, 0xb5, 0x42,
                    0x25, 0x6c, 0x36, 0x41, 0x22, 0xb,  0xa3, 0xc6, 0xef, 0xb7,
                    0x3f, 0x1f, 0x67, 0x5}};

  assert_memory_equal(&expectDecoratedSignature4,
                      feeBumpTransactionEnvelope.signatures + 1,
                      sizeof(struct DecoratedSignature));
}
bool _fee_bump_transaction_envelope_to_xdr(
    const struct FeeBumpTransactionEnvelope *feeBumpTransactionEnvelope,
    char **buf, size_t *buf_size) {
  stellarxdr_TransactionEnvelope transactionEnvelope_xdr;
  if (!fee_bump_transaction_envelope_to_xdr_object(feeBumpTransactionEnvelope,
                                                   &transactionEnvelope_xdr)) {
    return false;
  }
...
void test_fee_bump_transaction_envelope_to_xdr_object(void **state) {
  char *account = "GB777MJSHL7QOVZ3S52M2MROORDKLSJQL6MUSBV57R2NW7LODWORELJO";
  struct MuxedAccount muxed_account;
  assert_true(muxed_account_from_address(account, &muxed_account));
  struct Memo memo;
  ...
  char *buf0 = NULL;
  size_t buf_size0 = 0;
  assert_true(_fee_bump_transaction_envelope_to_xdr(&feeBumpTransactionEnvelope,
                                                    &buf0, &buf_size0));
  char xdr[] = {
      0x0,  0x0,  0x0,  0x5,  0x0,  0x0,  0x0,  0x0,  0xd7, 0xc9, 0xd,  0xa4,
      0x2b, 0xca, 0x8e, 0x1f, 0xef, 0x72, 0x7f, 0x78, 0x39, 0x7b, 0xea, 0x9,
      0x1a, 0x55, 0xb2, 0x23, 0xb9, 0x2a, 0x16, 0x7,  0x37, 0xad, 0x4e, 0xa9,
      0x2f, 0xd5, 0x85, 0xaa, 0x0,  0x0,  0x0,  0x0,  0x0,  0x0,  0x3,  0xe8,
      ...
      0xc6, 0x20, 0x8f, 0x5,  0x0,  0x0,  0x0,  0x0,  0x0,  0x0,  0x0,  0x2,
      0x2f, 0xd5, 0x85, 0xaa, 0x0,  0x0,  0x0,  0x40, 0x13, 0x17, 0xfe, 0x6a,
      0xd8, 0x41, 0x6f, 0xe4, 0x1c, 0x19, 0xa0, 0xca, 0x6d, 0xdd, 0xd1, 0x8a,
      0xf7, 0xe7, 0x84, 0x79, 0x15, 0x69, 0xd7, 0xa9, 0x8f, 0xfb, 0xdd, 0x95,
      0x80, 0x6f, 0x68, 0xed, 0xd4, 0x53, 0xec, 0x7b, 0xf,  0xdc, 0xa9, 0x88,
      0xea, 0xa7, 0xf5, 0xc3, 0xd,  0x60, 0xaa, 0x2e, 0x95, 0x53, 0xd3, 0x86,
      0x64, 0x64, 0x7e, 0xb1, 0x3b, 0x61, 0x39, 0xd,  0x31, 0x0,  0xe6, 0x2,
      0x9e, 0xf1, 0x96, 0xaa, 0x0,  0x0,  0x0,  0x40, 0x1e, 0xc3, 0x50, 0x51,
      0x32, 0x28, 0x25, 0x8b, 0xa5, 0x2a, 0x11, 0x8d, 0x14, 0x86, 0x6d, 0x6d,
      0xc2, 0xb8, 0x63, 0x92, 0xd2, 0x39, 0xa8, 0x52, 0x93, 0x68, 0x2a, 0xf7,
      0x67, 0x90, 0x2c, 0x92, 0x52, 0x3e, 0x79, 0xf6, 0x89, 0xb7, 0x78, 0x24,
      0x47, 0x33, 0xc9, 0x8d, 0x33, 0x15, 0xc4, 0x8c, 0xb5, 0x42, 0x25, 0x6c,
      0x36, 0x41, 0x22, 0xb,  0xa3, 0xc6, 0xef, 0xb7, 0x3f, 0x1f, 0x67, 0x5};
  assert_int_equal(buf_size0, 468);
  assert_memory_equal(buf0, xdr, buf_size0);

  stellarxdr_TransactionEnvelope to;
  assert_true(fee_bump_transaction_envelope_to_xdr_object(
      &feeBumpTransactionEnvelope, &to));
  struct FeeBumpTransactionEnvelope from;
  assert_true(fee_bump_transaction_envelope_from_xdr_object(
      &to, &from, PUBLIC_NETWORK_PASSPHRASE));
  char *buf1 = NULL;
  size_t buf_size1 = 0;
  assert_true(_fee_bump_transaction_envelope_to_xdr(&from, &buf1, &buf_size1));
  assert_int_equal(buf_size1, 468);
  assert_memory_equal(buf1, xdr, buf_size1);
}

int main() {
  const struct CMUnitTest tests[] = {
      cmocka_unit_test(test_fee_bump_transaction_envelope_hash),
      cmocka_unit_test(test_fee_bump_transaction_envelope_sign_with_one_signer),
      cmocka_unit_test(
          test_fee_bump_transaction_envelope_sign_with_two_signers),
      cmocka_unit_test(test_fee_bump_transaction_envelope_to_xdr_object),
  };
