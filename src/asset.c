...
bool asset_to_trust_line_asset_xdr_object(const struct Asset *in,
  ...
  case ASSET_TYPE_CREDIT_ALPHANUM4:
    out->type = stellarxdr_ASSET_TYPE_CREDIT_ALPHANUM4;
    memset(out->stellarxdr_TrustLineAsset_u.alphaNum4.assetCode, 0, 4);
    memcpy(out->stellarxdr_TrustLineAsset_u.alphaNum4.assetCode, in->code,
           strlen(in->code));
    memcpy(&out->stellarxdr_TrustLineAsset_u.alphaNum4.issuer, &accountId,
           sizeof(stellarxdr_AccountID));
    break;
  case ASSET_TYPE_CREDIT_ALPHANUM12:
    out->type = stellarxdr_ASSET_TYPE_CREDIT_ALPHANUM12;
    memset(out->stellarxdr_TrustLineAsset_u.alphaNum12.assetCode, 0, 12);
    memcpy(out->stellarxdr_TrustLineAsset_u.alphaNum12.assetCode, in->code,
           strlen(in->code));
    memcpy(&out->stellarxdr_TrustLineAsset_u.alphaNum12.issuer, &accountId,
           sizeof(stellarxdr_AccountID));
    break;
  default:
    return false;
...
bool asset_from_trust_line_asset_xdr_object(const stellarxdr_TrustLineAsset *in,
  ...
  case stellarxdr_ASSET_TYPE_CREDIT_ALPHANUM4:
    out->type = ASSET_TYPE_CREDIT_ALPHANUM4;
    if (!encode_ed25519_public_key(&in->stellarxdr_TrustLineAsset_u.alphaNum4
                                        .issuer.stellarxdr_PublicKey_u.ed25519,
                                   out->issuer)) {
      return false;
    }
    out->code =
        malloc(strlen(in->stellarxdr_TrustLineAsset_u.alphaNum4.assetCode) + 1);
    memcpy(out->code, in->stellarxdr_TrustLineAsset_u.alphaNum4.assetCode,
           strlen(in->stellarxdr_TrustLineAsset_u.alphaNum4.assetCode));

    break;
  case stellarxdr_ASSET_TYPE_CREDIT_ALPHANUM12:
    out->type = ASSET_TYPE_CREDIT_ALPHANUM12;
    if (!encode_ed25519_public_key(&in->stellarxdr_TrustLineAsset_u.alphaNum12
                                        .issuer.stellarxdr_PublicKey_u.ed25519,
                                   out->issuer)) {
      return false;
    }
    out->code = malloc(
        strlen(in->stellarxdr_TrustLineAsset_u.alphaNum12.assetCode) + 1);
    memcpy(out->code, in->stellarxdr_TrustLineAsset_u.alphaNum12.assetCode,
           strlen(in->stellarxdr_TrustLineAsset_u.alphaNum12.assetCode));
    break;
  default:
    return false;
  }
...
bool asset_to_change_trust_asset_xdr_object(const struct Asset *in,
  ...
  case ASSET_TYPE_CREDIT_ALPHANUM4:
    out->type = stellarxdr_ASSET_TYPE_CREDIT_ALPHANUM4;
    memset(out->stellarxdr_ChangeTrustAsset_u.alphaNum4.assetCode, 0, 4);
    memcpy(out->stellarxdr_ChangeTrustAsset_u.alphaNum4.assetCode, in->code,
           strlen(in->code));
    memcpy(&out->stellarxdr_ChangeTrustAsset_u.alphaNum4.issuer, &accountId,
           sizeof(stellarxdr_AccountID));
    break;
  case ASSET_TYPE_CREDIT_ALPHANUM12:
    out->type = stellarxdr_ASSET_TYPE_CREDIT_ALPHANUM12;
    memset(out->stellarxdr_ChangeTrustAsset_u.alphaNum12.assetCode, 0, 12);
    memcpy(out->stellarxdr_ChangeTrustAsset_u.alphaNum12.assetCode, in->code,
           strlen(in->code));
    memcpy(&out->stellarxdr_ChangeTrustAsset_u.alphaNum12.issuer, &accountId,
           sizeof(stellarxdr_AccountID));
    break;
  default:
    return false;
...
bool asset_from_change_trust_asset_xdr_object(
  ...
  case stellarxdr_ASSET_TYPE_CREDIT_ALPHANUM4:
    out->type = ASSET_TYPE_CREDIT_ALPHANUM4;
    if (!encode_ed25519_public_key(&in->stellarxdr_ChangeTrustAsset_u.alphaNum4
                                        .issuer.stellarxdr_PublicKey_u.ed25519,
                                   out->issuer)) {
      return false;
    }
    out->code = malloc(
        strlen(in->stellarxdr_ChangeTrustAsset_u.alphaNum4.assetCode) + 1);
    memcpy(out->code, in->stellarxdr_ChangeTrustAsset_u.alphaNum4.assetCode,
           strlen(in->stellarxdr_ChangeTrustAsset_u.alphaNum4.assetCode));

    break;
  case stellarxdr_ASSET_TYPE_CREDIT_ALPHANUM12:
    out->type = ASSET_TYPE_CREDIT_ALPHANUM12;
    if (!encode_ed25519_public_key(&in->stellarxdr_ChangeTrustAsset_u.alphaNum12
                                        .issuer.stellarxdr_PublicKey_u.ed25519,
                                   out->issuer)) {
      return false;
    }
    out->code = malloc(
        strlen(in->stellarxdr_ChangeTrustAsset_u.alphaNum12.assetCode) + 1);
    memcpy(out->code, in->stellarxdr_ChangeTrustAsset_u.alphaNum12.assetCode,
           strlen(in->stellarxdr_ChangeTrustAsset_u.alphaNum12.assetCode));
    break;
  default:
    return false;
  }
